<x-app-layout>
    ...
    {{-- 
        ...
        Mengatur logic Modal, Search, dan Scroll Spy dalam satu object besar
    --}}
    <div x-data="{ 
         ...
         "
         class="bg-gray-50 min-h-screen pb-32 relative"> {{-- pb-32 untuk memberi ruang Sticky Bar --}}

        {{-- 1. HEADER KONTEKS + SEARCH BAR --}}
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        @foreach($category['items'] as $item)
                            ...
                            <div class="group bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-xl transition-all duration-300 flex flex-row sm:flex-col h-36 sm:h-auto cursor-pointer"
                                 ...
                                 x-transition
                                 @click="openModal({{ json_encode($itemData) }})"> {{-- TRIGGER MODAL DISINI --}}
                                ...
                                <div class="w-1/3 sm:w-full sm:h-52 relative bg-gray-100 overflow-hidden">
                                    ...
                                    @if(($item->ketersediaanHariIni->jumlah_saat_ini ?? 0) < 10)
                                        <div class="absolute top-2 left-2 bg-red-600 text-white text-[10px] font-bold px-2 py-0.5 rounded shadow-sm">Sisa {{ $item->ketersediaanHariIni->jumlah_saat_ini ?? 0 }}</div>
                                    @endif
                                </div>

                                <div class="w-2/3 sm:w-full p-4 flex flex-col justify-between">
                                    <div>
                                        <h3 class="font-bold text-gray-900 text-lg leading-tight mb-1 line-clamp-1">{{ $item->namaMenu }}</h3>
                                        <p class="text-xs text-gray-500 line-clamp-2 mb-2 hidden sm:block">{{ $item->deskripsi }}</p>
                                    </div>
                                    <div class="flex items-end justify-between mt-auto">
                                        <span class="font-black text-gray-900 text-lg">Rp {{ number_format($item->harga, 0, ',', '.') }}</span>
        ...
        {{-- 
            =============================================
            COMPONENT 1: STICKY BOTTOM CART BAR (KFC STYLE)
            =============================================
        --}}
        @auth
            ...
            <div class="container mx-auto px-4 py-3">
                <div class="flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-0">
                    ...
                    <div class="flex items-center justify-between w-full sm:w-auto gap-6">
                        ...
                        <a href="{{ route('cart.list') }}" class="flex items-center gap-3 bg-kfc-red hover:bg-red-700 text-white px-6 py-3 rounded-full font-bold shadow-lg shadow-red-200 transition-all active:scale-95">
                            <i data-lucide="shopping-cart" class="w-5 h-5"></i>
                            <span>Checkout</span>
                            <span class="bg-white text-kfc-red text-xs font-bold px-2 py-0.5 rounded-full ml-1">{{ \Cart::getTotalQuantity() }} item(s)</span>
                        </a>
                    </div>
                </div>
            </div>
        ...
        <template x-teleport="body">
            <div 
                x-show="modalOpen" 
                style="display: none;" {{-- Mencegah flash saat loading --}}
