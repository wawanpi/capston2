<x-app-layout>
    ...
    <div x-data="{ 
        ...
        {{-- 1. HERO BANNER (REVISI: BACKGROUND IMAGE + OVERLAY) --}}
        <div class="relative pt-12 pb-20 px-4 rounded-b-[3rem] shadow-2xl overflow-hidden min-h-[400px] flex items-center">
            
            {{-- A. BACKGROUND IMAGE --}}
            {{-- Gunakan gambar Nila Bakar atau gambar Hero yang Anda miliki --}}
            <div class="absolute inset-0 z-0">
                {{-- Placeholder Image (Nila Bakar) - Ganti src ini dengan {{ asset('img/hero-bg.jpg') }} --}}
            ...
            {{-- B. DARK OVERLAY --}}
            {{-- Gradasi Hitam Transparan agar tulisan terbaca jelas --}}
                ...
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-8">
                    ...
                    <div class="text-white max-w-xl">
                        <div class="flex items-center gap-2 mb-3">
                            ...
                            <p class="text-gray-300 text-sm font-medium">Halo, {{ Auth::user()->name ?? 'Sobat Burmin' }}! 👋</p>
                        </div>
        ...
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
            @foreach($categories as $category)
                <section id="{{ $category['id'] }}" class="menu-section scroll-mt-48 transition-all duration-500">
                    <div class="flex items-center gap-3 mb-5">
                        {{-- Indikator Kategori Merah --}}
                        ...
                        @foreach($category['items'] as $item)
                            ...
                            <div class="group bg-white rounded-2xl p-3 shadow-sm border border-gray-100 hover:shadow-xl hover:border-burmin-yellow transition-all duration-300 cursor-pointer flex flex-row md:flex-col gap-4 h-32 md:h-auto relative overflow-hidden"
                                ...
                                <div class="w-28 md:w-full h-full md:h-48 rounded-xl overflow-hidden relative shrink-0 bg-gray-100">
                                    <img src="{{ asset($item->gambar) }}" alt="{{ $item->namaMenu }}" 
                                ...
                                <div class="flex flex-col justify-between flex-grow py-1">
                                    <div>
                                        <div class="flex justify-between items-start gap-2">
                                            ...
                                            @if(($item->reviews_avg_rating ?? 0) > 0)
                                        ...
                                        <p class="text-xs text-gray-400 mt-1 line-clamp-2">{{ $item->deskripsi }}</p>
                                    </div>
                                    ...
                                    <div class="flex items-center justify-between mt-2 md:mt-4">
                                        ...
                                        <button class="w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center transition-all active:scale-90 shadow-sm"
                                                :class="{{ $stok <= 0 ? '!bg-gray-100 !text-gray-300 cursor-not-allowed' : 'bg-gray-100 text-gray-900 group-hover:bg-burmin-red group-hover:text-white' }}"
        ...
        {{-- 4. FLOATING CART ISLAND --}}
        @auth
        <div x-show="{{ \Cart::getTotalQuantity() > 0 ? 'true' : 'false' }}" 
             class="fixed bottom-6 left-4 right-4 z-40 md:max-w-md md:left-auto md:right-8"
             x-transition:enter="transform transition ease-out duration-300"
             x-transition:enter-start="translate-y-20 opacity-0"
             x-transition:enter-end="translate-y-0 opacity-100"
             x-cloak>
             {{-- Cart Background Hitam (Logo Dominan) --}}
            <a href="{{ route('cart.list') }}" class="flex items-center justify-between bg-burmin-black/95 backdrop-blur-md text-white p-4 rounded-2xl shadow-2xl shadow-gray-400/50 border border-burmin-yellow/50 hover:bg-black transition-all group">
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex flex-col">
                        ...
                        <span class="font-bold text-xl leading-none">Rp {{ number_format(\Cart::getTotal(), 0, ',', '.') }}</span>
                    </div>
                </div>
