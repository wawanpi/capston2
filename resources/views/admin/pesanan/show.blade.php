    ...
    <div class="py-12 no-print">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            @if ($message = Session::get('success'))
                <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative mb-4" role="alert"><span class="block sm:inline">{{ $message }}</span></div>
            @endif
            @if ($message = Session::get('warning'))
                <div class="bg-yellow-100 border border-yellow-400 text-yellow-800 px-4 py-3 rounded relative mb-4" role="alert"><strong class="font-bold">Perhatian!</strong> <span class="block sm:inline">{{ $message }}</span></div>
            @endif
            @if ($message = Session::get('error'))
                <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-4" role="alert"><strong class="font-bold">Gagal!</strong> <span class="block sm:inline">{{ $message }}</span></div>
            @endif
            @if ($errors->any())
                            ...
                            @if($pesanan->bukti_bayar)
                                ...
                                <div class="flex flex-col items-start">
                                    <div class="border rounded p-2 bg-gray-50 inline-block mb-3">
                                        <img src="{{ asset($pesanan->bukti_bayar) }}" alt="Bukti Transfer" class="max-h-80 w-auto object-contain rounded-md shadow-sm">
                                    </div>
