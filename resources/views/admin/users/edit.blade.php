                ...
                <div class="p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('admin.users.update', $user->id) }}">
                        ...
                        <div>
                            ...
                            <x-text-input id="name" class="block mt-1 w-full border-gray-300 focus:border-red-500 focus:ring-red-500 rounded-md shadow-sm" type="text" name="name" :value="old('name', $user->name)" required autofocus autocomplete="name" />
                        </div>

                        {{-- Email: Menambahkan class untuk override focus color --}}
                        <div class="mt-4">
                            <x-input-label for="email" :value="__('Email')" />
                            <x-text-input id="email" class="block mt-1 w-full border-gray-300 focus:border-red-500 focus:ring-red-500 rounded-md shadow-sm" type="email" name="email" :value="old('email', $user->email)" required autocomplete="username" />
                        </div>

                        {{-- Password (Opsional): Menambahkan class untuk override focus color --}}
                        <div class="mt-4">
                            <x-input-label for="password" :value="__('Password Baru (Kosongkan jika tidak diubah)')" />
                            <x-text-input id="password" class="block mt-1 w-full border-gray-300 focus:border-red-500 focus:ring-red-500 rounded-md shadow-sm" type="password" name="password" autocomplete="new-password" />
                        </div>
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="password_confirmation" class="block mt-1 w-full border-gray-300 focus:border-red-500 focus:ring-red-500 rounded-md shadow-sm" type="password" name="password_confirmation" autocomplete="new-password" />
                        </div>
                        ...
                        <div class="mt-4">
                             ...
                             <select name="roles" id="roles" class="block mt-1 w-full border-gray-300 focus:border-red-500 focus:ring-red-500 rounded-md shadow-sm" required>
                                 ...
                                 @foreach($roles as $role)
                                     <option value="{{ $role }}" {{ old('roles', $userRole) == $role ? 'selected' : '' }}>
                                         {{ $role }}
                                     </option>
                                 @endforeach
                             </select>
                        ...
                        <div class="flex items-center justify-end mt-6">
                            ...
                            {{-- Tombol Update: Diubah dari <x-primary-button> menjadi <button> Merah --}}
