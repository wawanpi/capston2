...
>
    ...
    <div class="p-4 flex items-center gap-3 border-b bg-gray-50">
        ...
        <div class="overflow-hidden">
            @auth
                <span class="font-semibold text-gray-800 text-sm block truncate">{{ Auth::user()->name }}</span>
                <span class="text-xs text-gray-500 block">Administrator</span>
            @else
                <a href="{{ route('login') }}" class="font-semibold text-gray-800 text-sm">Login</a>
            @endauth
        </div>
    </div>

    <nav class="flex-grow p-4 space-y-2 overflow-y-auto">
        <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-3 p-3 rounded-lg font-semibold hover:bg-red-50 hover:text-kfc-red transition-colors {{ request()->routeIs('admin.dashboard') ? 'bg-red-50 text-kfc-red border-r-4 border-kfc-red' : 'text-gray-600' }}">
            <i data-lucide="layout-dashboard" class="w-5 h-5"></i>
            <span>Dashboard</span>
        </a>
        
        <a href="{{ route('admin.menus.index') }}" class="flex items-center gap-3 p-3 rounded-lg font-semibold hover:bg-red-50 hover:text-kfc-red transition-colors {{ request()->routeIs('admin.menus.*') ? 'bg-red-50 text-kfc-red border-r-4 border-kfc-red' : 'text-gray-600' }}">
            <i data-lucide="utensils-crossed" class="w-5 h-5"></i>
            <span>Manajemen Menu</span>
        </a>

        <a href="{{ route('admin.pesanan.index') }}" class="flex items-center gap-3 p-3 rounded-lg font-semibold hover:bg-red-50 hover:text-kfc-red transition-colors {{ request()->routeIs('admin.pesanan.*') ? 'bg-red-50 text-kfc-red border-r-4 border-kfc-red' : 'text-gray-600' }}">
            <i data-lucide="shopping-bag" class="w-5 h-5"></i>
            <span>Kelola Pesanan</span>
        </a>

        <a href="{{ route('admin.transaksi.index') }}" class="flex items-center gap-3 p-3 rounded-lg font-semibold hover:bg-red-50 hover:text-kfc-red transition-colors {{ request()->routeIs('admin.transaksi.*') ? 'bg-red-50 text-kfc-red border-r-4 border-kfc-red' : 'text-gray-600' }}">
            <i data-lucide="receipt" class="w-5 h-5"></i>
            <span>Kelola Transaksi</span>
        </a>

        <a href="{{ route('admin.users.index') }}" class="flex items-center gap-3 p-3 rounded-lg font-semibold hover:bg-red-50 hover:text-kfc-red transition-colors {{ request()->routeIs('admin.users.*') ? 'bg-red-50 text-kfc-red border-r-4 border-kfc-red' : 'text-gray-600' }}">
        ...
        <div class="border-t pt-4 mt-2">
            <form method="POST" action="{{ route('logout') }}" class="w-full">
                @csrf
                <a href="{{ route('logout') }}" onclick="event.preventDefault(); this.closest('form').submit();" class="flex items-center gap-3 p-3 rounded-lg text-red-600 hover:bg-red-50 w-full transition-colors">
            ...
            <div class="relative">
                ...
                >
                    ...
                    <nav class="p-2 text-gray-700 text-sm">
                        <a href="{{ route('profile.edit') }}" class="flex items-center gap-3 px-3 py-2 rounded hover:bg-gray-100">
