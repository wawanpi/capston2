    ...
    <div x-data="{ 
            ...
            <div class="container mx-auto">
                <nav class="flex overflow-x-auto no-scrollbar py-3 px-4 gap-3 sm:gap-6 snap-x scroll-pl-4">
                    @foreach($categories as $cat)
                        <a href="#{{ $cat['id'] }}" 
                           @click.prevent="document.getElementById('{{ $cat['id'] }}').scrollIntoView({behavior: 'smooth'})"
                           class="whitespace-nowrap px-5 py-2 rounded-full text-sm font-bold transition-all duration-300 snap-start border"
                           :class="activeSection === '{{ $cat['id'] }}' ? 'bg-black text-white border-black shadow-md scale-105' : 'bg-gray-50 text-gray-500 border-gray-200 hover:bg-gray-100'">
                            {{ $cat['name'] }}
                        </a>
                    @endforeach
                </nav>
        ...
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-16">
            @foreach($categories as $category)
                <section id="{{ $category['id'] }}" class="menu-section scroll-mt-40 transition-all duration-500">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        @foreach($category['items'] as $item)
                            ...
                            <div class="group bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-xl transition-all duration-300 flex flex-row sm:flex-col h-36 sm:h-auto cursor-pointer"
                                 x-show="!searchQuery || '{{ strtolower($item->namaMenu) }}'.includes(searchQuery.toLowerCase())"
                                 x-transition
                                 @click="openModal({{ json_encode($itemData) }})"> 
                                
                                {{-- Image Section --}}
                                <div class="w-1/3 sm:w-full sm:h-52 relative bg-gray-100 overflow-hidden">
                                    <img src="{{ asset($item->gambar) }}" alt="{{ $item->namaMenu }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" onerror="this.src='https://placehold.co/400x300/f3f4f6/a3a3a3?text=No+Image'">
                                    
                                    {{-- OVERLAY JIKA HABIS (SOLD OUT) --}}
                                    @if($stokSaatIni <= 0)
                                    ...
                                    @elseif($stokSaatIni < 10)
                                        <div class="absolute top-2 left-2 bg-red-600 text-white text-[10px] font-bold px-2 py-0.5 rounded shadow-sm z-10">Sisa {{ $stokSaatIni }}</div>
                                    @endif
                                    
                                    {{-- Rating Badge di Card --}}
                                    <div class="absolute bottom-2 left-2 bg-white/90 backdrop-blur-sm px-1.5 py-0.5 rounded-md flex items-center gap-1 shadow-sm z-10">
                                        ...
                                        <span class="text-[10px] font-bold text-gray-800">{{ number_format($item->reviews_avg_rating ?? 0, 1) }}</span>
                                    </div>
                                </div>
                                ...
                                <div class="w-2/3 sm:w-full p-4 flex flex-col justify-between">
                                    <div>
                                        <h3 class="font-bold text-gray-900 text-lg leading-tight mb-1 line-clamp-1 {{ $stokSaatIni <= 0 ? 'text-gray-400' : '' }}">{{ $item->namaMenu }}</h3>
                                        <p class="text-xs text-gray-500 line-clamp-2 mb-2 hidden sm:block">{{ $item->deskripsi }}</p>
                                    </div>
                                    <div class="flex items-end justify-between mt-auto">
                                        <span class="font-black text-gray-900 text-lg {{ $stokSaatIni <= 0 ? 'text-gray-400' : '' }}">Rp {{ number_format($item->harga, 0, ',', '.') }}</span>
                                        
                                        {{-- Tombol Plus (Disable jika habis) --}}
                                        <button class="{{ $stokSaatIni <= 0 ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-gray-100 hover:bg-red-600 text-gray-900 hover:text-white' }} rounded-full w-10 h-10 flex items-center justify-center transition-all shadow-sm active:scale-95"
                                                {{ $stokSaatIni <= 0 ? 'disabled' : '' }}>
                ...
                <div class="flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-0">
                    ...
                    <div class="flex items-center justify-between w-full sm:w-auto gap-6">
                        <div class="flex flex-col text-right">
                            ...
                            <span class="text-xl font-black text-gray-900">Rp {{ number_format(\Cart::getTotal(), 0, ',', '.') }}</span>
                        </div>
                        <a href="{{ route('cart.list') }}" class="flex items-center gap-3 bg-red-600 hover:bg-red-700 text-white px-6 py-3 rounded-full font-bold shadow-lg shadow-red-200 transition-all active:scale-95">
                            <i data-lucide="shopping-cart" class="w-5 h-5"></i>
                            <span>Checkout</span>
                            <span class="bg-white text-red-600 text-xs font-bold px-2 py-0.5 rounded-full ml-1">{{ \Cart::getTotalQuantity() }} item(s)</span>
                        </a>
                    </div>
                </div>
            </div>
